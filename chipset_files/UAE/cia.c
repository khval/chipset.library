...
void WriteCIAA(uae_u16 addr,uae_u8 val)
{
	...
	switch(addr & 0xf)
	{
		case 0:					// pra
			...
			// bit 6 mouse, we allow setting mouse in chipset.library, as way to control programs.

			ciaapra = (ciaapra & ~(1<<6 | 0x3)) | (val & (1<<6 | 0x3)); 
